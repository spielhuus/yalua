...
local RUN_TESTS = ""
local EXCLUDE_TAGS = "--exclude-tags='skip'"
...
local function spec_decode(data)
  ...
  table.insert(result, 'local dkjson = require("dkjson")')
  table.insert(result, 'local yalua = require("yalua")')
  table.insert(result, "")
  table.insert(result, 'describe("Run the YAML test #suite, compare with in.json ", function()\n')
  ...
  -- UPDATED: JSON Loader with stream support and correct null sentinel
  table.insert(result, "  local function load_json(file_path)")
  table.insert(result, "    local content, err = load_file(file_path)")
  table.insert(result, '    assert(content, "Failed to load JSON file: " .. file_path)')
  table.insert(result, "    local results = {}")
  table.insert(result, "    local pos = 1")
  table.insert(result, "    local len = #content")
  table.insert(result, "    while pos <= len do")
  -- PASS yalua.null as the 3rd argument so dkjson uses it instead of nil
  table.insert(result, "      local obj, next_pos, err = dkjson.decode(content, pos, yalua.null)")
  table.insert(result, "      if obj ~= nil then")
  table.insert(result, "        table.insert(results, obj)")
  table.insert(result, "        pos = next_pos")
  table.insert(result, "      else")
  table.insert(result, '        local remaining = content:sub(pos):match("^%s*$")')
  table.insert(result, "        if remaining then break end")
  table.insert(result, '        error("JSON parse error at " .. pos .. ": " .. (err or "unknown"))')
  table.insert(result, "      end")
  table.insert(result, "    end")
  -- Always return a list (sequence of docs) to match { yalua.decode() } structure
  table.insert(result, "    return results")
  table.insert(result, "  end")
  ...
  for _, testfile in ipairs(data) do
    ...
    for _, test in ipairs(testfile) do
      ...
      local json = string.format("%s/data/%s/in.json", PATH_SUITE, remove_extension(filename))
      if #testfile > 9 then
        file = string.format("%s/data/%s/%03d/in.yaml", PATH_SUITE, remove_extension(filename), test_nr)
        json = string.format("%s/data/%s/%03d/in.json", PATH_SUITE, remove_extension(filename), test_nr)
      elseif #testfile > 1 then
        file = string.format("%s/data/%s/%02d/in.yaml", PATH_SUITE, remove_extension(filename), test_nr)
        json = string.format("%s/data/%s/%02d/in.json", PATH_SUITE, remove_extension(filename), test_nr)
      end
      local fail = test["fail"] and true or false

      -- Change: Skip positive tests that do not have a corresponding in.json file
      if not fail and not dir_exists(json) then
        skip = true
      end
      ...
      if not skip then
        ...
        if fail then
          ...
          -- We are guaranteed to have json here because we skipped if !dir_exists(json)
          table.insert(result, string.format('    local expected = load_json("%s")', json))
          -- Capture unpacked yalua results into a table to compare lists
          table.insert(result, string.format("    local actual = { yalua.decode(input) }"))
          table.insert(result, string.format("    assert.is.Same(expected, actual)"))
  ...
  table.insert(result, "end)")
  write(PATH_TESTS .. "/decode_spec.lua", result)
  print(
    "[INFO] Tests generated: Positive: " .. total_tests .. ", Negative: " .. total_fails .. ", Skipped: " .. total_skip
  )
end
...
local function suite()
  ...
  prepare_suite()
  if os.execute(CMD_LUAROCKS .. "test spec/suite/ -- " .. RUN_TESTS) ~= 0 then
...
local function test()
  clone_test_suite()
  if os.execute(CMD_LUAROCKS .. " test spec/test -- " .. EXCLUDE_TAGS .. " --defer-print") ~= 0 then
    error("Tests did not run successfully")
  end
end

local function check()
  if os.execute("luacheck *.lua") ~= 0 then
...
local function print_usage()
  ...
  print("  diff <filename> - diff a yaml dump with libfyaml")
end
...
if is_main(arg, ...) then
  ...
  elseif arg[1] == "dump" then
    ...
    local stream, mes = yalua.dump(get_file_content(arg[2]))
    if not stream then
      print(mes)
    else
      print(stream)
  ...
  elseif arg[1] == "diff" then
    ...
    if not stream then
      ...
      write_file_content("/tmp/yalua.dump", stream)
    end
    os.execute("libfyaml/build/fy-tool -C off --testsuite " .. arg[2] .. " > /tmp/fy-tool.dump")
    os.execute("nvim -d /tmp/fy-tool.dump /tmp/yalua.dump")
  else
    print_usage()
  end
else

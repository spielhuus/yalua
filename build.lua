...
local function suite()
  ...
  prepare_suite()
  if os.execute(CMD_LUAROCKS .. "test spec/suite/tree_spec.lua -- " .. RUN_TESTS) ~= 0 then
...
local function test()
  clone_test_suite()
  if os.execute(CMD_LUAROCKS .. " test spec/test -- --exclude-tags='skip' --defer-print") ~= 0 then
...
if is_main(arg, ...) then
  ...
  elseif arg[1] == "diff" then
    ...
    os.execute("../yalua/libfyaml/src/fy-tool -C off --testsuite " .. arg[2] .. " > /tmp/fy-tool.dump")

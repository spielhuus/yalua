		...
		elseif c == "\\" then
			...
			elseif next_c == "U" then
				...
				if not val then
					error(
						string.format(
							"ERROR:%d:%d Invalid escape sequence '\\%s'",
							self.line,
							self:current_col(),
							next_c or "EOF"
						)
		...
		elseif c == "\n" then
			...
			while true do
				...
				if self:get_line_indent() == 0 then
					if
						(self:match(0, "---") and self:is_separator(3))
						or (self:match(0, "...") and self:is_separator(3))
					then
						error(
							string.format(
								"ERROR:%d:%d invalid document-start marker in double-quoted scalar",
								self.line,
								1
							)
						)
					...
					if self.flow_level == 0 then
						...
						if is_multi_line_start then
							if current_indent < start_line_indent then
								error(
									string.format(
										"ERROR:%d:%d wrongly indented double-quoted scalar",
										self.line,
										current_indent
									)
								)
			...
			while true do
				...
				if self:get_line_indent() == 0 then
					...
					then
						error(
							string.format(
								"ERROR:%d:%d invalid document-start marker in single-quoted scalar",
								self.line,
								1
							)
						)
					...
					if self.flow_level == 0 then
						...
						if is_multi_line_start then
							if current_indent < start_line_indent then
								error(
									string.format(
										"ERROR:%d:%d wrongly indented single-quoted scalar",
										self.line,
										current_indent
									)
								)
	...
	if not token then
		...
		while not self:is_eof() do
			...
			if self:at(":") then
				...
				if
					self:is_separator(1)
					or (self.flow_level > 0 and (is_flow_indicator or next_c == "[" or next_c == "{"))
				then
					break
				end
			end
		...
		if token.kind == "QUESTION" then
			...
			then
				...
				if token.kind == "D_QUOTE" then
					...
					table.insert(
						self.tokens,
						{ kind = "VAL", val = tok.value, style = '"', tag = key_tag, anchor = key_anchor }
					)
				elseif token.kind == "S_QUOTE" then
					local tok = self.lexer:next()
					table.insert(
				...
				elseif token.kind == "TAG" then
					...
					table.insert(
						self.tokens,
						{ kind = "VAL", val = key_str, style = ":", tag = key_tag, anchor = key_anchor }
					)
				end
	...
	while true do
		...
		if token.kind == "DOC_END" then
			...
			if self.lexer:peek().kind == "DOC_END" then
				...
				local next_token = self.lexer:peek()
				if
					next_token.row == doc_end_token.row
					and next_token.kind ~= "NEWLINE"
